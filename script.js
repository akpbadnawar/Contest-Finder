let url = 'https://kontests.net/api/v1/all'
let response = fetch(url)
response.then((v) => {
  return v.json()
}).then((contests) => {
  console.log(contests)
  ihtml = ''
  for (item in contests) {
    // console.log(contests[item]) //commeting out the API response
    ihtml += `
    <div class="card container-sm my-2 mx-2 bg-alert text-white bg-dark" style="width: 20rem;">
        <img src="https://www.trio.dev/hubfs/laptop-sand-clock.webp" class="card-img-top" alt="...">
        <div class="card-body">
          <h5 class="card-title">${contests[item].name} and site is ${contests[item].site} </h5>
          <p class='card-text'> Status: ${contests[item].status}</p>
          <p class='card-text'> In 24 Hours: ${contests[item].in_24_hours}</p>
          <p> Starts at: ${contests[item].start_time}
          <p> Ends at: ${contests[item].end_time}
          <a href="${contests[item].url}" class="btn btn-primary my-4">Visit Contest</a>
        </div>
    </div>`

  }
  cardContainer.innerHTML = ihtml
})